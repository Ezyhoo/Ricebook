...
getFollowing = async(req, res) => {
    ...
    if(user === null || user === undefined){
        res.status(200).send(`Cannot find user: ${username}`);
    }
    else{
...
addFollowing = async(req, res) => {
    ...
    if(user.following.includes(req.params.user)){
        res.status(200).send(`You have already followed ${req.params.user}`);
        return;
    }
...
deleteFollowing = async(req, res) => {
    ...
    if(newFollowing.length === user.following.length){
        res.status(200).send(`${req.params.user} is not in the following`);
        return;
    }
    const updatedUser = await profileModel.findOneAndUpdate(
        {username: loggedInUser.username},
