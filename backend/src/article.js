...
updateArticle = async(req, res) => {
    try{
        ...
        if(article === null || article === undefined){
            res.status(200).send(`No post with id ${id}`);
            return;
        }
        else if(article.author !== loggedInUser.username && req.body.commentId !== undefined && req.body.commentId != -1){
            res.sendStatus(401)
            return;
        }
            ...
            if(commentId === -1){
                ...
                if(idx === -1){
                    res.status(200).send(`No commentId with ${commentId}`);
                    return;
                }
